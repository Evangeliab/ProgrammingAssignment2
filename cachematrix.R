...
cacheSolve <- function(x, ...) {
        ...
        if(!is.null(s)) {    # If the inverse has already been calculated             
                message("getting cached data")  # (and the matrix has not  
                return(s)    # changed),then the cachesolve should retrieve the 
        }          # inverse from the cache.Loop prints a message and the value.
        
        data <- x$get()         ## accesses the original matrix.
        s <- solve(data, ...)  ## calculates the inverse matrix.
        x$setsolve(s)        ## stores the calculated value in makeCacheMatrix.
